...
opt.PLB = [0, 0, 0, 0];
opt.PUB = [500, 10, 500, 500];
opt.LB = [0, 0, 0, 0];
opt.UB = [500, 20, 500, 500];
...
%% fit
fitResult = zeros(numel(brainSite), nSubj, nFit, nFreeParams+3); % two parameter + maxlh, AIC,BIC
for iSubj = 1:nSubj  % loop subject
    for iSite = 1:numel(brainSite) % 
        for iFit = 1:nFit %loop fit
            ...
            x0_tmp=x0(iFit,:);            
            [fitparams,maxlh,AIC,BIC] = fit_VP_model_rz(N, probe, resp, x0_tmp);
