function [fitpars, posilh, AIC, BIC] = fit_cosSA_model(N, probe, resp, x0)
...
% <posilh>,<AIC>,<BIC>: postive likelihood value,AIC, BIC values
...
posilh = fval; % here max_lh is a positive value, a real likelihood value should be -max_lh
%fitpars = X_mat(LLH==max_lh,:);
fitpars = x;
% compute AIC and BIC
n_free_pars = gvar.n_par;   % Jm, K, Jf, muf 
BIC = -2*-posilh + n_free_pars*log(numel(data.N));
AIC = -2*-posilh + 2*n_free_pars;
