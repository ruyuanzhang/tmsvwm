...
opt.PLB = [0, 0, 0, 0];
opt.PUB = [10, 8, 500, 2*pi];
opt.LB = [0, 0, 0, 0];
opt.UB = [10, 8, 500, 2*pi];
...
nFreeParams = size(x0,2);

%% fit normal subjects
fitResult = zeros(numel(brainSite), nSubj, nFit, nFreeParams+3); % four parameter + maxlh, AIC,BIC
for iSubj = 1:nSubj  % loop subject
    for iSite = 1:numel(brainSite) % 
        for iFit = 1:nFit %loop fit
            ...
            x0_tmp=x0(iFit,:);            
            [fitparams,maxlh,AIC,BIC] = fit_SAcos_model(N, probe, resp, x0_tmp);
