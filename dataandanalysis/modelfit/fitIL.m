...
%% fit normal subjects
fitResult = zeros(numel(brainSite), nSubj, nFit, nFreeParams+3); % parameters + maxlh, AIC,BIC
for iSubj = 1:nSubj  % loop subject
    for iSite = 1:numel(brainSite) % 
        for iFit = 1:nFit %loop fit
            ...
            x0_tmp=x0(iFit,:);            
            [fitparams,maxlh,AIC,BIC] = fit_IL_model(N, probe, resp, x0_tmp);
